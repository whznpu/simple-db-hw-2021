...
public class Aggregate extends Operator {
    ...
    public Aggregate(OpIterator child, int afield, int gfield, Aggregator.Op aop) {
        // some code goes here
        // child 是经过聚合后的child 构造时需要调用merge方法
        // check afield's type
        ...
        if(aFieldType==Type.INT_TYPE){
            IntegerAggregator agg= new IntegerAggregator(gfield,gbFieldType,afield,aop);
            while(true){
                try {
            ...
            StringAggregator agg= new StringAggregator(gfield,gbFieldType,afield,aop);
            while(true){
                try {
                    if (!child.hasNext()) break;
                    agg.mergeTupleIntoGroup(child.next());
